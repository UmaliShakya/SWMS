...
def predict_water_consumption_paddy(reservoir, water_levels):
    df = read_frame(water_levels, fieldnames=[
        'date', 'water_consumption_paddy', 'rainfall', "temperature", 'evaporation'], index_col=['date'])

    df = df.tail(n_steps_in)

    scaled_data = df

    f_columns = ['water_consumption_paddy',
    ...
    print(df[['water_consumption_paddy']])

    print(
        final_output[['water_consumption_paddy']])
