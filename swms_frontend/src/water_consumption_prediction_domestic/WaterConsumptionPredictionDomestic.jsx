...
const Chart = ({ data_real, data_predicted, reservoir }) => {
    ...
    return (
        <>
            <Title>{`Water Consumption Prediction Domestic - ${reservoir.name}`}</Title>
                ...
                <Table size="small">
                    <TableHead>
                        <TableRow>
                            ...
                            <TableCell>Water Consumption Domestic (Gallons)</TableCell>
                        </TableRow>
                    </TableHead>
                    <TableBody>
                        {rows_real.map((row) => (
                        ...
                        {rows_predicted.map((row) => (
                            <TableRow key={row.id} className={classes.tableRow}>
                                <TableCell>{row.date}</TableCell>
                                <TableCell>{row.water_consumption_domestic}</TableCell>                           </TableRow>
                        ))}
                    </TableBody>
                </Table>
            </TableContainer>
...
const WaterConsumptionPredictionDomestic = (props) => {
    const classes = useStyles();
    ...
    const { data, loading, error } = useQueryWithStore({
        type: 'getOne',
        resource: 'water_consumption_prediction_domestic',
        payload: { id: id }
    });
    ...
    return (
        <Card>
            <AdminTitle title={`Water Consumption Prediction Domestic - ${data.reservoir.name}`} />
            <CardContent>
...
export default WaterConsumptionPredictionDomestic;